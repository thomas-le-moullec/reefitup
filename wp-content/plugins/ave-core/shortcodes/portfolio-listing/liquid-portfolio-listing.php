	...
	public function get_params() { 
		...
		$general = array(
			...
			array(
				...
				'description' => esc_html__( 'Add posts, pages, etc. by title.', 'ave-core' ),
				'settings'    => array(
					'multiple' => true,
					'sortable' => true,
					'groups'   => true,
		...
		$data = array(
			...
			array(
				...
				'description' => esc_html__( 'Exclude posts, pages, etc. by title.', 'ave-core' ),
				'group'       => esc_html__( 'Data Settings', 'ave-core' ),
				'settings'    => array(
					'multiple' => true,
					'no_hide'  => true, // In UI after select doesn't hide an select list
...
*/
	...
	protected function get_overlay_button() {
		...
		if( 'listing-lightbox-gallery' === $enable_gallery ) {
			...
			printf( '<a href="%s" class="liquid-overlay-link fresco" data-fresco-group="'. esc_attr( $this->get_id() ) .'"></a>', $url );	
		}
		else {
			printf( '<a href="%s" %s class="liquid-overlay-link"></a>', $url, $target );	
		}
