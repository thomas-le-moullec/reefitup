...
<div id="reviews" class="woocommerce-Reviews">
	<div id="comments">
		<h2 class="woocommerce-Reviews-title"><?php
			if ( get_option( 'woocommerce_enable_review_rating' ) === 'yes' && ( $count = $product->get_review_count() ) ) {
				...
				printf( esc_html( _n( '%1$s review for %2$s', '%1$s reviews for %2$s', $count, 'ave' ) ), esc_html( $count ), '<span>' . get_the_title() . '</span>' );
			} else {
				_e( 'Reviews', 'ave' );
			}
		?></h2>
	...
	<?php else : ?>
		...
		<p class="woocommerce-verification-required"><?php _e( 'Only logged in customers who have purchased this product may leave a review.', 'ave' ); ?></p>
